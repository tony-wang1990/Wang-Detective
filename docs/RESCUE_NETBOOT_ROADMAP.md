...
netboot.xyz 已作为 W-探长“救援中心”的受保护实验能力接入，但不会提供无确认的一键清盘或重装。
...
3. **netboot.xyz 引导实验区**
   - 适用于用户明确知道自己要网络安装/救援，并且机器仍可 SSH、使用 UEFI 和 OCI 控制台。
   - 当前可通过已保存 SSH 主机一键预检，下载对应 AMD/ARM EFI，创建 UEFI 启动项并设置一次性 BootNext。
   - 默认只准备不重启；立即重启必须输入 `NETBOOT-REBOOT`。
